...
**未プッシュのコミットがある場合**:
1. `git push -u origin <現在のブランチ名>` を実行する
2. 「未プッシュのコミットをプッシュしました」とユーザーに報告する

**未プッシュがない場合**: スキップ（何も言わない）
...
3. ヒットしたノートを読む
4. 読み取った内容を踏まえて回答する
5. **「AIに覚えさせたいリスト」を確認する**（後述 セクション9参照）
...
2. 未処理エントリがあれば、URLの内容をWebフェッチで取得する
3. 内容を要約してObsidianの `Knowledge/` に保存する（ファイル名: `YYYY-MM-DD-記事タイトルのキーワード.md`）
4. Notionのステータスを「保存済み」に、Obsidianファイル欄に保存先ファイル名を記入する
5. ユーザーに「〜件の記事を処理してObsidianに保存しました」と報告する
...
## AKATOSHIチャンネルへの応用
（投資・AI・ビジネス関連の場合、YouTube活用アイデアを記載）
```

---

## 10. Notion・連携アプリへの登録ルール（必須）

NotionページやNotionデータベース、その他連携アプリにコンテンツを登録・更新する際は、以下を**必ず**実行すること：

1. **最終更新日をページの一番上に記載する**
   - 形式: `**最終更新日: YYYY-MM-DD**`
   - 区切り線（`---`）をその直後に入れる
...
3. **更新時は最終更新日と更新履歴の両方を更新する**

> このルールはNotionページ・Notionデータベースのエントリ・その他連携アプリ（将来追加分も含む）すべてに適用する。
