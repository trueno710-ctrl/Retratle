...
- どのフォルダを作ったか
- 各フォルダの役割
- 最初に書いておくと良いもの（例: 「自己紹介を Preferences/profile.md に書いておくと覚えやすくなります」）

このセクションは初回のみ実行し、通常の会話では参照しない。

---

## 1. 読み取り（セッション開始時に必ず実行）

新しい会話の最初のメッセージで以下を実行:

1. `Knowledge/mistakes.md` と `Preferences/` 配下を最初に読み込む
...
セッション中にユーザーから訂正を受け、かつ以下3条件をすべて満たすときのみ追記:
